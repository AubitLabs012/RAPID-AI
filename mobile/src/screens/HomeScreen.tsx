...
import { Box, Crosshair, Layers, Mic, Search, UserRound } from "lucide-react";
  ...
  return (
    <button
      ...
      className={cn(
        ...
        active ? "bg-blue-600 text-white ring-2 ring-blue-300" : "bg-white text-slate-800",
      )}
    >
      {children}
    </button>
...
function Toggle({ label, checked, onChange }: { label: string; checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <label className="flex items-center justify-between border-b border-line py-3 text-[15px]">
      {label}
      <input type="checkbox" checked={checked} onChange={(e) => onChange(e.target.checked)} className="size-5 accent-blue-600" />
    </label>
  );
}
